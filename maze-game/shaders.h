...
// Other platforms: use GLSL 1.50 with in/out (typical with GL 3.2+ contexts).
...
// Unlit pass (skybox, floor, billboards): only texture coordinates vary; avoids link warnings.
...
void main() {
    // Reference vs_in_norm so the compiler keeps attribute location 1 (VAO still supplies normals).
